...
class JacoCommandReceiverTestBase : public testing::Test {
 public:
  ...
  VectorXd finger_position() const {
    return dut_.get_commanded_finger_position_output_port().Eval(context_);
  }

  VectorXd finger_velocity() const {
    return dut_.get_commanded_finger_velocity_output_port().Eval(context_);
  }
...
TEST_F(JacoCommandReceiverTest, AcceptanceTestWithoutMeasuredPositionInput) {
  ...
  const VectorXd zero = VectorXd::Zero(kJacoDefaultArmNumJoints);
  EXPECT_TRUE(CompareMatrices(position(), zero));
  EXPECT_TRUE(CompareMatrices(velocity(), zero));

  const VectorXd finger_zero = VectorXd::Zero(kJacoDefaultArmNumFingers);
  EXPECT_TRUE(CompareMatrices(finger_position(), finger_zero));
  EXPECT_TRUE(CompareMatrices(finger_velocity(), finger_zero));
}
...
TEST_F(JacoCommandReceiverTest,
  ...
  dut_.get_finger_position_measured_input_port().FixValue(&context_, f_q0);
  EXPECT_TRUE(CompareMatrices(position(), q0));
  EXPECT_TRUE(CompareMatrices(velocity(), zero));
  EXPECT_TRUE(CompareMatrices(finger_position(), f_q0));
  EXPECT_TRUE(CompareMatrices(finger_velocity(), finger_zero));
  ...
  EXPECT_TRUE(CompareMatrices(position(), q1));
  EXPECT_TRUE(CompareMatrices(velocity(), v1));
  EXPECT_TRUE(CompareMatrices(finger_position(), f_q1 * kFingerSdkToUrdf));
  EXPECT_TRUE(CompareMatrices(finger_velocity(), f_v1 * kFingerSdkToUrdf));
}
...
TEST_F(JacoCommandReceiverTest, AcceptanceTestWithLatching) {
  ...
  // command is to hold at the current position.
  const VectorXd q0 = VectorXd::LinSpaced(N, 0.0, 0.1);
  const VectorXd f_q0 = VectorXd::LinSpaced(N_F, 1.0, 1.1);
  dut_.get_position_measured_input_port().FixValue(&context_, q0);
  dut_.get_finger_position_measured_input_port().FixValue(&context_, f_q0);
  EXPECT_TRUE(CompareMatrices(position(), q0));
  EXPECT_TRUE(CompareMatrices(velocity(), zero));
  ...
  // Prior to any update events, changes to position_measured feed through.
  const VectorXd q1 = VectorXd::LinSpaced(N, 0.1, 0.2);
  const VectorXd f_q1 = VectorXd::LinSpaced(N_F, 1.1, 1.2);
  dut_.get_position_measured_input_port().FixValue(&context_, q1);
  dut_.get_finger_position_measured_input_port().FixValue(&context_, f_q1);
  EXPECT_TRUE(CompareMatrices(position(), q1));
  EXPECT_TRUE(CompareMatrices(velocity(), zero));
  EXPECT_TRUE(CompareMatrices(finger_position(), f_q1));
  EXPECT_TRUE(CompareMatrices(finger_velocity(), finger_zero));
  ...
  dut_.LatchInitialPosition(&context_);
  const VectorXd q2 = VectorXd::LinSpaced(N, 0.3, 0.4);
  const VectorXd f_q2 = VectorXd::LinSpaced(N_F, 1.3, 1.4);
  dut_.get_position_measured_input_port().FixValue(&context_, q2);
  dut_.get_finger_position_measured_input_port().FixValue(&context_, f_q2);
  EXPECT_TRUE(CompareMatrices(position(), q1));
  EXPECT_TRUE(CompareMatrices(velocity(), zero));
  EXPECT_TRUE(CompareMatrices(finger_position(), f_q1));
  EXPECT_TRUE(CompareMatrices(finger_velocity(), finger_zero));
  ...
  EXPECT_TRUE(CompareMatrices(position(), q3));
  EXPECT_TRUE(CompareMatrices(velocity(), v3));
  EXPECT_TRUE(CompareMatrices(finger_position(), f_q3 * kFingerSdkToUrdf));
  EXPECT_TRUE(CompareMatrices(finger_velocity(), f_v3 * kFingerSdkToUrdf));
}
