...
class AcrobotModelTests : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    AddModelFromUrdfFile(full_name, plant_.get());
...
// loaded with the URDF parser.
class MultibodyPlantUrdfParser : public ::testing::Test {
 public:
  void SetUp() override {
    const std::string kUrdfPath =
        "drake/multibody/multibody_tree/parsing/test/"
        "links_with_visuals_and_collisions.urdf";
    full_name_ = FindResourceOrThrow(kUrdfPath);
  }

  // Loads the MultibodyPlant part of the model. Geometry is ignored.
  void LoadMultibodyPlantOnly() {
    AddModelFromUrdfFile(full_name_, &plant_);
    plant_.Finalize();
  }
  ...
  void LoadMultibodyPlantAndSceneGraph() {
    AddModelFromUrdfFile(full_name_, &plant_, &scene_graph_);
...
TEST_F(MultibodyPlantUrdfParser, LinkWithVisuals) {
...
// supplied.
TEST_F(MultibodyPlantUrdfParser, ParseWithoutASceneGraph) {
...
// call to AddModelFromUrdfFile().
TEST_F(MultibodyPlantUrdfParser, RegisterWithASceneGraphBeforeParsing) {
...
TEST_F(MultibodyPlantUrdfParser, LinksWithCollisions) {
  LoadMultibodyPlantAndSceneGraph();
